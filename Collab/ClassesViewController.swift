...
class ClassesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var classObjects = NSMutableArray()
    ...
    override func viewDidLoad() {
        ...
        self.classesTableView.contentInset = adjustForTabbarInsets
        self.classesTableView.scrollIndicatorInsets = adjustForTabbarInsets
        ...
        classesTableView.tableFooterView = footerView
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        print("Loading Cell")
        if let classObject = self.classObjects[indexPath.row] as? [String: Any] {
            let title = classObject["title"] as! String
            let teacher = classObject["teacherName"] as! String
            print(classObject)
                ...
                Firestore.firestore().collection("classes").document(className).getDocument() { (document, err) in
                    if let err = err {
                        ...
                        if let document = document, document.exists {
                            ...
                            print("Added item")
                            self.classObjects.add(dataDescription)
                        } else {
                            print("Document does not exist")
                        }
                        if i == GlobalVariables.classes!.count {
                            self.stopLoading()
                            print("ClassesVC: Reloading")
